...
function getMeasureKey(riskMeasures: RiskMeasuresForAssets) {
    return getKey(
        riskMeasures.key.hazard_type,
        riskMeasures.key.scenario_id,
        riskMeasures.key.year,
        riskMeasures.key.measure_id
    )
}
...
export class CalculationResult {
    ...
    constructor(calculationResult: any) {
        ...
        this.riskMeasuresHelper = new RiskMeasuresHelper(
            calculationResult.risk_measures
        )
...
export class RiskMeasuresHelper {
    ...
    constructor(riskMeasures: RiskMeasures) {
        ...
        this.measures = {}
        riskMeasures.measures_for_assets.forEach(
            (m) => (this.measures[getMeasureKey(m)] = m)
        )
    }

    measure(hazardType: string, scenarioId: string, year: number) {
        const measure_key = getKey(
            hazardType,
            scenarioId,
            year.toString(),
            this.measure_set_id()
        ...
        const measureIds =
            this.measure_definition().asset_measure_ids_for_hazard[hazardType]
        // measure definitions for each asset
        const measureDefinitions = measureIds.map((mid) =>
            mid != "na"
                ? this.measure_definition().score_definitions[mid]
                : null
        )
        return {
            assetScores: assetScores,
            assetMeasures: assetMeasures,
...
export function createDataTable(
    ...
    let table: {
        [key: string]: string | number | { [key: string]: ScoreDetails }
    }[] = []
    Object.keys(hazardMap).forEach((hazardType) => {
        let row: {
            [key: string]: string | number | { [key: string]: ScoreDetails }
        } = { hazard: hazardMap[hazardType], id: hazardMap[hazardType] }
        table.push(row)
...
export function createBarChartData(
    ...
    let hazardTypeKey: any = Object.entries(hazardMap).find(
        (item) => item[1] === hazardType
    )?.[0]
    years.forEach((year) => {
...
export function createHazardImpact(
    result: CalculationResult,
    assetIndex: number,
    hazardType: string,
    scenarioId: string
    ...
    allYears.forEach((y) => {
        let impacts = assetImpacts.find(
            (i) =>
                hazardMap[i.key.hazard_type] === hazardType &&
                i.key.scenario_id ===
                    (y === "historical" ? "historical" : scenarioId) &&
        ...
        if (impacts) {
            ...
            if (impacts!.calc_details?.hazard_exceedance) {
                ...
                let hazardValues: number[] =
                    impacts!.calc_details.hazard_exceedance.values
                let [hazardProbsCapped, hazardValuesCapped] = capCurve(
                    hazardProbs,
                    hazardValues,
                    0.001,
                    false
                )
                let hazardData = hazardProbsCapped
                    .map((item, i) =>
                        graphDataPoint(
                            1.0 / item, // expects return periods
    ...
    Object.keys(hazardMap).forEach((hazardType) => {
        ...
        if (measure.assetScores.length > 0) {
            if (scores.length === 0) {
                ...
                scores = measure.assetScores.map((s, i) =>
                    Math.max(s, scores[i])
                )
            }
        }
...
function capCurve(
    ...
    if (
        extrapolate &&
        probs.length > probsCapped.length &&
        probsCapped[probsCapped.length - 1] != probCap
    ) {
        // we need to add prob 0.001 by linear interpolation
        let index = probsCapped.length - 1
        let value =
            values[index] +
            ((probCap - probs[index]) * (values[index + 1] - values[index])) /
