...
export default function HazardViewer(props) {
    ...
    async function fetchGraphData() {
      if (hazardMenu.inventory)
      {
        if (lngLat)
        {
          ...
          var points = (curve_set.intensities.length == 1) ? [graphDataPoint(0, curve_set.intensities[0])]
            : curve_set.return_periods.map((item, i) => graphDataPoint(1.0/ item, curve_set.intensities[i]));
  ...
  if (graphData)
  {
    var title = hazardMenu.menuOptions ? hazardMenu.menuOptions[1][hazardMenu.selectedIndices[1]] + 
    ...
    {
      chart = (<ChronicHazard title={title} data={graphData} units={hazardMenu.selectedModel.units} />)
    }
  }
  else chart = (<div></div>)
        ...
        <Grid item xs={12} md={4} lg={3}>
          ...
          >
            <Summary 
              ...
              modelDescription={hazardMenu?.selectedModel?.description} 
            />
          </Paper>
        </Grid>
      </Grid>
